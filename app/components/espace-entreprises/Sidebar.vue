...
import { espaceEntreprisesNavItems } from '@/data/espace-entreprises/current-entreprise'
...
const isActiveRoute = (href: string) => {
  ...
  if (href === '/espace-entreprises/dashboard') {
    return route.path === '/espace-entreprises/dashboard' || route.path === '/espace-entreprises'
  }

  // Verifier si une autre route de navigation est plus specifique
  const hasMoreSpecificMatch = espaceEntreprisesNavItems.some(item => {
  ...
  >
    ...
    <div class="h-16 flex items-center justify-between px-4 border-b border-gray-200 dark:border-repae-gray-700">
      <NuxtLink
        ...
        @click="emit('closeMobile')"
      >
        <div class="h-10 w-10 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center flex-shrink-0">
          <font-awesome-icon icon="fa-solid fa-building" class="text-white text-lg" />
        </div>
        <span
          v-if="!isCollapsed || isMobileOpen"
          class="text-lg font-bold font-brand text-emerald-600 dark:text-emerald-400"
        >
          Espace Recruteur
        </span>
      </NuxtLink>
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3">
      <ul class="space-y-1">
        <li v-for="item in espaceEntreprisesNavItems" :key="item.id">
          <NuxtLink
            :to="item.href"
            :class="[
            ...
            <div
              :class="[
                ...
                isActiveRoute(item.href)
                  ...
                  : 'bg-gray-100 dark:bg-repae-gray-800 text-repae-gray-500 dark:text-repae-gray-400 group-hover:bg-emerald-100 dark:group-hover:bg-emerald-500/20 group-hover:text-emerald-500'
              ]"
            >
