...
// Get offre data
const offre = computed(() => getOffreById(offreId))

// Redirect if offre not found
if (!offre.value) {
  router.push('/espace-entreprises/offres')
}

useHead({
  title: computed(() => offre.value ? `Candidatures - ${offre.value.titre} | REPAE` : 'Candidatures | REPAE')
})

// Get candidatures for this offre
const candidatures = ref<Candidature[]>(getCandidaturesByOffre(offreId, 'emploi'))
const stats = computed(() => getCandidaturesStats(offreId, 'emploi'))
    ...
    <div class="mb-8">
      ...
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p v-if="offre" class="text-repae-gray-500 dark:text-repae-gray-400 mt-1">
            {{ offre.titre }}
          </p>
        </div>
    ...
    >
      <div class="flex items-center gap-4">
        <div class="w-12 h-12 bg-emerald-100 dark:bg-emerald-500/20 rounded-xl flex items-center justify-center">
          <font-awesome-icon icon="fa-solid fa-briefcase" class="text-emerald-500 text-xl" />
        </div>
        <div class="flex-1">
        ...
        <NuxtLink
          :to="`/espace-entreprises/offres`"
          class="px-4 py-2 text-sm text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-lg transition-colors"
        >
          Voir l'offre
        </NuxtLink>
      </div>
        ...
        <div class="relative bg-white dark:bg-repae-gray-800 rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div>
              ...
              <div class="flex flex-wrap gap-2">
                <span
                  ...
                  :key="comp"
                  class="px-3 py-1 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 rounded-full text-sm"
                >
                  {{ comp }}
                </span>
              </div>
          ...
          <div class="sticky bottom-0 bg-white dark:bg-repae-gray-800 border-t border-gray-200 dark:border-repae-gray-700 p-6 flex flex-wrap gap-3">
            ...
            <button
              ...
              type="button"
              class="flex-1 px-4 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-medium font-brand text-sm transition-colors cursor-pointer flex items-center justify-center gap-2"
